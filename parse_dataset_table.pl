...
foreach my $d (sort {$a cmp $b} keys %Peakset) {
	...
	for(my$i=0;$i<@IPs;$i++){
		print OUT2 "# peak calls: original replicates and self-pesduoreplicates $IPs[$i]
			Rscript $Config{'bin_dir'}/spp_package/run_spp.wig.R -c=$IPs[$i].tagAlign.gz -i=$d\_Input_Rep0.tagAlign.gz -npeak=$npeaks -odir=./ -p=$node -savr -savp -rf -out=$Config{'date'}.cc
			...
			Rscript $bin_dir/spp_package/run_spp.R -c=$IPs[$i].pr1.tagAlign.gz -i=$d\_Input_Rep0.tagAlign.gz -npeak=$npeaks -odir=./ -p=$node -savr -savp -rf -out=$Config{'date'}.cc
			Rscript $Config{'bin_dir'}/spp_package/run_spp.R -c=$IPs[$i].pr2.tagAlign.gz -i=$d\_Input_Rep0.tagAlign.gz -npeak=$npeaks -odir=./ -p=$node -savr -savp -rf -out=$Config{'date'}.cc
			\n";
	...
	print OUT2 "# peak calls: pooled
		Rscript $Config{'bin_dir'}/spp_package/run_spp.wig.R -c=$d\_IP_Rep0.tagAlign.gz -i=$d\_Input_Rep0.tagAlign.gz -npeak=$npeaks -odir=./ -p=$node -savr -savp -rf -out=$Config{'date'}.cc
	...
	print OUT2 "# peak calls: pooled-pseduoreplicates
		Rscript $Config{'bin_dir'}/spp_package/run_spp.R -c=$d\_IP_Rep0.pr1.tagAlign.gz -i=$d\_Input_Rep0.tagAlign.gz -npeak=$npeaks -odir=./ -p=$node -savr -savp -rf -out=$Config{'date'}.cc
		Rscript $Config{'bin_dir'}/spp_package/run_spp.R -c=$d\_IP_Rep0.pr2.tagAlign.gz -i=$d\_Input_Rep0.tagAlign.gz -npeak=$npeaks -odir=./ -p=$node -savr -savp -rf -out=$Config{'date'}.cc
		\n";
